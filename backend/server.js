  ...
  try {
    ...
    if (isDuplicate) {
      return res.status(409).json({
        success: false,
        message: `คุณได้บันทึกข้อมูลค่าน้ำตาล "${type} มื้อ${period}" ในวันที่ ${date} ไปแล้ว กรุณาเลือกช่วงเวลาอื่น`,
        isDuplicate: true,
      });
    }
...
// === Endpoint สำหรับเรียก cron จาก Render ===
app.get("/run-reminder", async (req, res) => {
  try {
    console.log("Manual trigger: running daily reminders...");
    await sendDailyReminders();
    res.json({ success: true, message: "Reminders sent successfully" });
  } catch (error) {
    console.error("Error running reminders manually:", error);
    res.status(500).json({ success: false, error: error.message });
  }
});

// ตั้งค่า Cron Job ให้ทำงาน
// cron.schedule(
//   "0 5 * * *",
//   () => {
